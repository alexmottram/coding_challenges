...
int main(int, char**)
{
    ...
    for (int current = 1; current <= 100; ++current) {
        if (current % 3 == 0) logger_three.log(std::format("Hit {}", current));
        if (current % 7 == 0) logger_seven.log(std::format("Hit {}", current));
        if (current % 13 == 0) logger_thirteen.log(std::format("Hit {}", current));
